...
def adiciona_equação(elemento):
    equacao = input.get()
    input.delete(0, END)
    input.insert(0, elemento if equacao == "" else equacao + elemento)
...
def calcular():
    ...
    print(equacao_npr)
    resultado = p.resolve_npr(equacao_normal)
    print(resultado)
    input.delete(0, END)
...
botao_limpar = Button(root, font=font_utilizada, padx=19, pady=20,
                  text="limpar", command=limpar)
botao_calcular = Button(root, font=font_utilizada, padx=350, pady=20,
                  text="=", command=calcular)

                
                


# Adiciona os botões na grid
